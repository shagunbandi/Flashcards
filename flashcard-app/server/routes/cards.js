...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (card.front_type === 'image' && card.front_content.startsWith('/uploads/')) {
      const filePath = path.join('/data', card.front_content);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    }
    if (card.back_type === 'image' && card.back_content.startsWith('/uploads/')) {
      const filePath = path.join('/data', card.back_content);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    }
